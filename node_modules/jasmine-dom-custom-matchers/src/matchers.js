...
export default class Matchers {
	_toBeHTMLElement() {
		return {
			compare: function (elem, tag) {
				...
				elemValid = _utils.isHTML(elem);
				tagValid = _utils.is(tag, 'String');
				...
				msgNot = "Expected " + _utils.getType(elem) + " not to be a " + msgTag + " Element";
				msgFail = "Expected " + _utils.getType(elem) + " to be a " + msgTag + " Element";
				return _utils.test(pass, msgNot, msgFail);
			}
		};
	};
	...
	_toBeHTMLText() {
		return {
			compare: function (elem, val) {
				...
				elemValid = _utils.is(elem, 'text');
				strValid = _utils.is(val, 'string');
				regValid = _utils.is(val, 'regexp');
				...
				msgStr = " of content '" + _utils.shortenStr(val) + "'";
				msgReg = " of content matching regular expression " + val;
				msgContent = elemValid && strValid ? msgStr : elemValid && regValid ? msgReg : "";
				msgNot = "Expected " + _utils.getType(elem) + " not to be the [HTML Text] Object" + msgContent;
				msgFail = "Expected " + _utils.getType(elem) + " to be the [HTML Text] Object" + msgContent;
				return _utils.test(pass, msgNot, msgFail);
			}
		};
	};
	...
	_toBeDocumentNode() {
		return _utils.returnBlock(function (o, not) {
			var elem = o[0], elemValid, domTest, pass, msgDOM, msg;
			elemValid = _utils.isHTML(elem) || _utils.is(elem, 'text');
			...
			msg = "Expected " + _utils.getType(elem) + msgDOM + _utils.notMsg(not) + " to be the document node";
			return _utils.test(pass, msg, msg);
		});
	};

	_toContainHTMLElement() {
		return _utils.returnBlock(function (o, not) {
			var elem = o[0], child = o[1], elemValid, childTest, pass, msg;
			elemValid = _utils.isHTML(elem) && _utils.isHTML(child);
			childTest = elemValid ? elem.contains(child) && elem !== child : false;
			pass = not ? elemValid && !childTest : elemValid && childTest;
			msg = "Expected " + _utils.getType(elem) + _utils.notMsg(not) + " to contain " + _utils.getType(child);
			return _utils.test(pass, msg, msg);
		});
	};
	...
	_toContainText() {
		return _utils.returnBlock(function (o, not) {
			...
			elemValid = _utils.isHTML(elem);
			strValid = _utils.is(text, 'string');
			regValid = _utils.is(text, 'regexp');
			...
			msgStr = "'" + _utils.shortenStr(text) + "'";
			msgReg = "regular expression " + text;
			msgUnvalid = _utils.getType(text) + " while text or regular expression was expected";
			msgText = strValid ? msgStr : regValid ? msgReg : msgUnvalid;
			msg = "Expected " + _utils.getType(elem) + _utils.notMsg(not) + " to contain " + msgText;
			return _utils.test(pass, msg, msg);
		});
	};
	...
	_toBeChildOf() {
		return _utils.returnBlock(function (o, not) {
			...
			paramValid = (_utils.isHTML(child) || _utils.is(child, 'text')) && _utils.isHTML(parent);
			paramTest = paramValid ? child.parentNode === parent : false;
			pass = not ? paramValid && !paramTest : paramValid && paramTest;
			msg = "Expected " + _utils.getType(child) + _utils.notMsg(not) + " to be the child element of " + _utils.getType(parent);
			return _utils.test(pass, msg, msg);
		});
	};
	...
	_toBeNthChild() {
		return _utils.returnBlock(function (o, not) {
			...
			elemValid = _utils.isHTML(elem);
			numValid = _utils.is(index, 'number') ? index >= 0 : false;
			strValid = _utils.is(index, 'string') ? Boolean(index.match(/\s*last\s*/i)) : false;
			valid = elemValid && (numValid || strValid);
			children = elemValid ? _utils.isHTML(elem.parentNode) ? elem.parentNode.children : [] : [];
			...
			msgNotParent = " while " + _utils.getType(elem) + " has not got parent Element";
			...
			msg = "Expected " + _utils.getType(elem) + _utils.notMsg(not) + " to be " + index + ending(index) + " child node of its parent" + msgAdd;
			return _utils.test(pass, msg, msg);
	...
	_toBeParentOf() {
		return _utils.returnBlock(function (o, not) {
			...
			paramValid = _utils.isHTML(parent) && (_utils.isHTML(child) || _utils.is(child, 'text'));
			paramTest = paramValid ? child.parentElement === parent : false;
			pass = not ? paramValid && !paramTest : paramValid && paramTest;
			msg = "Expected " + _utils.getType(child) + _utils.notMsg(not) + " to be the parent element of " + _utils.getType(parent);
			return _utils.test(pass, msg, msg);
		});
	};
	...
	_toHaveSameParent() {
		return _utils.returnBlock(function (o, not) {
			...
			childAValid = _utils.isHTML(childA) || _utils.is(childA, 'text');
			childBValid = _utils.isHTML(childB) || _utils.is(childB, 'text');
			...
			msg = "Expected " + _utils.getType(childA) + _utils.notMsg(not) + " to have the same parent as " + _utils.getType(childB);
			return _utils.test(pass, msg, msg);
		});
	};
	...
	_toHaveChildren() {
		return _utils.returnBlock(function (o, not) {
			...
			parentValid = _utils.isHTML(parent);
			opValid = ops.some(findOperator);
			numValid = parentValid ? _utils.is(num, 'Number') && num >= 0 : false;
			...
			msg = "Expected " + _utils.getType(parent) + _utils.notMsg(not) + " to contain " + msgNum + msgAdd;
			return _utils.test(pass, msg, msg);
	...
	_toBeNextSiblingOf() {
		return _utils.returnBlock(function (o, not) {
			...
			sibValid = _utils.isHTML(sibA) && _utils.isHTML(sibB);
			next = sibValid ? sibB.nextElementSibling : next;
			nextTest = sibValid ? next === sibA : false;
			pass = not ? sibValid && !nextTest : sibValid && nextTest;
			msgNext = " while next sibling is " + _utils.getType(next);
			msgNull = " while " + _utils.getType(sibB) + " has not got next sibling element";
			msgAdd = not ? "" : next ? msgNext : sibValid ? msgNull : "";
			msg = "Expected " + _utils.getType(sibA) + _utils.notMsg(not) + " to be next sibling of " + _utils.getType(sibB) + msgAdd;
			return _utils.test(pass, msg, msg);
		});
	};

	_toBePreviousSiblingOf() {
		return _utils.returnBlock(function (o, not) {
			...
			msgPrev = " while previous sibling is " + _utils.getType(prev);
			msgNull = " while " + _utils.getType(sibB) + " has not got previous sibling element";
			msgAdd = not ? "" : prev ? msgPrev : sibValid ? msgNull : "";
			msg = "Expected " + _utils.getType(sibA) + _utils.notMsg(not) + " to be previous sibling of " + _utils.getType(sibB) + msgAdd;
			return _utils.test(pass, msg, msg);
		});
	};
	...
	_toBeEmpty() {
		return _utils.returnBlock(function (o, not) {
			...
			msg = "Expected " + _utils.getType(elem) + _utils.notMsg(not) + " to be empty" + msgAdd;
			return _utils.test(pass, msg, msg);

			function collection() {
				var nodes = elem.childNodes, parsed = [];
				for (var i = 0; i < nodes.length; i++) {
					if (_utils.isHTML(nodes[i])) {
	...
	_toHaveAnyAttribute() {
		return _utils.returnBlock(function (o, not) {
			...
			msg = "Expected " + _utils.getType(elem) + _utils.notMsg(not) + " to have any attributes defined";
			return _utils.test(pass, msg, msg);
		});
	};
	...
	_toHaveAttribute() {
		return _utils.returnBlock(function (o, not) {
			...
			elemValid = _utils.isHTML(elem);
			attrValid = _utils.is(attr, 'string');
			strValid = _utils.is(val, 'string');
			regValid = _utils.is(val, 'regex');
			...
			msgAttr = !elemValid ? "" : attrValid ? " '" + attr + "'" : ' of type ' + _utils.getType(attr);
			msgVal = !elemValid || !attrValid ? "" : strValid ? " of '" + val + "' value " : regValid ? " of value matched regular expression " + val : "";
			msg = "Expected " + _utils.getType(elem) + _utils.notMsg(not) + " to have an attribute" + msgAttr + msgVal + " specified";
			return _utils.test(pass, msg, msg);
		});
	};
	...
	_toHaveClass() {
		return _utils.returnBlock(function (o, not) {
			...
			elemValid = _utils.isHTML(elem);
			clssValid = _utils.is(clss, 'string');
			valid = elemValid && clssValid;
			clssTest = valid ? hasClass(elem) : false;
			pass = not ? valid && !clssTest : valid && clssTest;
			msgAdd = !elemValid ? "" : clssValid ? " '" + clss + "'" : " of type " + _utils.getType(clss);
			msg = "Expected " + _utils.getType(elem) + _utils.notMsg(not) + " to have class" + msgAdd;
			return _utils.test(pass, msg, msg);
	...
	_toHaveComputedStyle() {
		return _utils.returnBlock(function (o, not) {
			...
			propValid = _utils.is(prop, 'string');
			strValid = _utils.is(val, 'string');
			regValid = _utils.is(val, 'regexp');
			...
			msgProp = !(elemValid && domValid) ? "" : propValid ? "'" + propHyphen + "'" : "[" + _utils.getType(prop) + "]";
			msgCssValue = !valid || !propTest ? "" : ", while the computed value is '" + cssValue + "'";
			msgVal = !(propValid && elemValid && domValid) ? "" : strValid ? " of the value '" + val + "'" : regValid ? " of the value matched the regular expression " + val : " of [" + _utils.getType(val) + "] value";
			msg = "Expected " + _utils.getType(elem) + msgDOM + _utils.notMsg(not) + " to have the " + msgProp + " style" + msgVal + msgCssValue;
			return _utils.test(pass, msg, msg);
		});
	};

	_toHaveComputedColor(util) {
		return _utils.returnBlock(function (o, not) {
			...
			msgProp = !(elemValid && domValid) ? "" : propValid ? "'" + propHyphen + "'" : "[" + _utils.getType(prop) + "]";
			msgCssValue = !valid || !propTest ? "" : ", while the computed value is '" + cssValue + "'";
			msgVal = !(propValid && elemValid && domValid) ? "" : valValid ? " of the value '" + val + "'" : " of [" + _utils.getType(val) + "] value";
			msg = "Expected " + _utils.getType(elem) + msgDOM + _utils.notMsg(not) + " to have the " + msgProp + " style" + msgVal + msgCssValue;
			return _utils.test(pass, msg, msg);
	...
	_toHaveEvent() {
		return _utils.returnBlock(function (o, not) {
			...
			elemValid = _utils.isHTML(elem);
			evntValid = _utils.is(evnt, 'string');
			...
			msg = "Expected " + _utils.getType(elem) + _utils.notMsg(not) + " to have " + msgEvent + " event attached";
			return _utils.test(pass, msg, msg);
